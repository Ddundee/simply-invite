...
export default function InvitationResponse({ id }: Props) {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        toast.loading('Responding to invitation...', { id: 'invitation-response' })
        console.log(values)
        try {
            ...
            toast.success('Successfully responded to invitation', { id: 'invitation-response' })
        }
        catch {
            toast.error('Failed to respond to invitation', { id: 'invitation-response' })
        }
    }
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 p-2">
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className={!form.getValues('coming') ? `text-muted` : ""}>Name</FormLabel>
                                    <FormControl>
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className={!form.getValues('coming') ? `text-muted` : ""}>How many people are coming with you?</FormLabel>
                                    <FormControl>
                                        <Input type='number' {...field} onChange={(event) => {
                                            field.onChange({ target: { value: parseInt(event.target.value) } })
                                        }} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
